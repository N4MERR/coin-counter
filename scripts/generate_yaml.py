...
def generate_yaml(dataset_path, split_data_path, test_data_path, output_file, classes_filename):
    """
    Reads class names from a text file in the original dataset and generates a YAML configuration pointing to the split dataset, including a custom test data path.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_data_path', type=str, required=True, help="Path to the test dataset directory")
    ...
    generate_yaml(args.path, args.split_data_path, args.test_data_path, args.output, args.classes)